defmodule Manifold do
  ...
  def split(%MapSet{} = splitters, row_idx, beams) when is_map(beams) do
    if MapSet.size(splitters) == 0 do
      ...
      beams
      |> Enum.filter(fn {{row, _}, _} -> row == row_idx end)
      |> Enum.reduce(beams, fn {{row, col}, timelines}, beams ->
        row = row + 1

        if {row, col} in splitters do
          beams
          |> Map.update({row, col - 1}, timelines, &(&1 + timelines))
          |> Map.update({row, col + 1}, timelines, &(&1 + timelines))
        else
          Map.update(beams, {row, col}, timelines, &(&1 + timelines))
        end
      end)
    end
  end
...
assert Manifold.p1(test_manifold) == 21
assert Manifold.p1(manifold) |> IO.inspect(label: :p1) == 1543
